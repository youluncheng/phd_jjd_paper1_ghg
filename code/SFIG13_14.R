...
file_path_read <- "/Users/dongjingjing/Desktop/GHG/FIG/SFIG/FIG13"
output_dir <- "/Users/dongjingjing/Desktop/GHG/FIG/SFIG/FIG13"
...
p1_final <- ggplot(long_data_sheet1, aes(x = Time_Period, y = Value)) +
  ...
  geom_segment(aes(x = Time_Period, xend = Time_Period, y = 0, yend = Value),
  ...
  labs(title = "", x = "", y = "Emission/Area (kg/ha)", color = "") +
  
 
  theme_compact_clean


ggsave(file.path(output_dir, "ghg_emission_area_clean_no_bottom_text.png"), 
       p1_final, width = 15, height = 20, unit="cm", dpi = 300)

print("Sheet 1 已保存 (移除底部年份文字)。")
...
ggsave(file.path(output_dir, "ghg_emission_sheet2_clean_no_bottom_text.png"), 
       p2_final, width = 15, height = 18, unit="cm", dpi = 300)

print("Sheet 2 已保存 (移除底部年份文字)。")